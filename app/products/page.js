...
export default function Pro() {
  ...
  const categories = ["All", "Beauty", "Groceries", "Furniture", "Sports"];
  ...
  return (
    <div className="flex flex-col">
      <ul className="flex p-10 justify-center">
        {categories.map((item, index) => (
          <li key={index} onClick={() => clickHandle(index)}>
            <button className="text-lg font-bold text-gray-700 hover:text-gray-900 transition duration-300 p-3">
              {item}
            </button>
          </li>
        ))}
      ...
      <div className="m-auto flex flex-wrap gap-5 justify-center">
        {filteredProducts.map(item => (
          <Link href={`/products/${item.id}`} key={item.id} className="w-48 p-5 border rounded hover:shadow-lg hover:shadow-black">
            <Image
              src={item.img}
              alt={item.name}
              width={190}
              ...
              className="mx-auto"
            />
            <h2 className="text-lg font-bold text-gray-700 hover:text-gray-900 transition">{item.name}</h2>
             <p className="text-center text-gray-600 mt-1">{item.price}</p>
              <p className="text-center text-yellow-500">Rating: {item.rating} ★</p>
